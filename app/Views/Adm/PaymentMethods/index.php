    ...
    <div class="col-lg-12 grid-margin stretch-card">
        <div class="card">
            ...
            <div class="card-body">
                ...
                <a href="<?php echo site_url('adm/payment/create'); ?>" class="btn btn-success float-right mt-1 mb-5">
                    ...
                    <table class="table table-hover">
                        ...
                        <tbody>
                            <?php foreach ($paymentMethods as $paymentMethod) { ?>
                                ...
                                <tr>
                                    <td>
                                        <a href="<?php echo site_url("adm/payment/show/$paymentMethod->id"); ?>"> <?php echo $paymentMethod->name; ?></a>
                                    </td>
                                 
                                    <td> <?php echo $paymentMethod->created_at->humanize(); ?></td>
                                    <td> <?php echo $paymentMethod->active && $paymentMethod->deleted_at == null ? '<label class="badge badge-primary">Sim</label>' : '<label class="badge badge-danger">Não</label>'; ?></td>
                                    <td> 
                                        <?php echo $paymentMethod->deleted_at == null ? '<label class="badge badge-primary">Disponível</label>' : '<label class="badge badge-danger"> Excluído </label>'; ?>
                                        <?php if ($paymentMethod->deleted_at != null) { ?>
                                            <a href="<?php echo site_url("adm/payment/undodelete/$paymentMethod->id"); ?>" class="badge badge-dark ml-2"> 
    ...
    $(function() {
        ...
        $("#query").autocomplete({
            source: function(request, response) {
                $.ajax({
                    url: "<?php echo site_url('adm/paymentmethods/search'); ?>",
                    dataType: "json",
                    data: {
                        term: request.term
                    },
            ...
            select: function(event, ui) {
                if (ui.item.values == -1) {
                    ...
                    window.location.href = '<?php echo site_url('adm/payment/show/'); ?>' + ui.item.id;
                }
            }
